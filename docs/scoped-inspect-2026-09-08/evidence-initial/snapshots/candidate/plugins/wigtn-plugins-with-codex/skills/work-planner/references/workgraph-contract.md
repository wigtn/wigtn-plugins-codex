...
python3 <plugin-root>/scripts/wigtn.py --json inspect --task TASK-ID
...
`--task TASK-ID` selects complete task records and transitive dependencies,
linked artifacts, checks, requirements and sources. Repeat it for multiple
tasks. Global validation, drift, summary and release gates remain visible;
`next_tasks` is scoped to the selection. Omission counts are explicit; shared
checks may reference tasks outside the selection. Unknown IDs fail. Omit
`--task` for the full view. `saved_revision` is the on-disk revision; the summary
revision may include an unpersisted drift preview.
