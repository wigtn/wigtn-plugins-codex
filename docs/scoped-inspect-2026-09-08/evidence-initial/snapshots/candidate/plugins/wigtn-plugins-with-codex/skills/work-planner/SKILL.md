...
- Read authoritative requirements and preserve their IDs. For existing state,
  ...
  currently eligible tasks in one read-only call. For a known task, append
  `--task TASK-ID` to retrieve its records and transitive dependencies while
  retaining global validation and drift warnings. The response states what
  was omitted; use the full view for graph-wide analysis. Reinspect after relevant
