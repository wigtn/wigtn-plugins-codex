...
def prepare(args):
    ...
    for arm,source in [('baseline',args.baseline),('candidate',REPO)]:m.snapshot_marketplace(source,args.root/'snapshots'/arm)
    shutil.copy2(__file__,args.root/'frozen-study.py')
    schedule=[];cases={};rng=random.Random(908731)
    for index,(family,n) in enumerate(CASES):
    ...
    protocol={'model':'gpt-6-astra','effort':'medium','max_calls':24,'timeout_seconds':300,'seed':908731,
    ...
    print(json.dumps({'root':str(args.root),'calls':24,'input_files':len(hashes(args.root))},indent=2))
...
def execute(args):
    ...
    for arm in ['bare','baseline','candidate']:
        ...
        if arm!='bare':
            ...
            installed=home/'plugins/cache/wigtn/wigtn-plugins-with-codex/0.5.4'
    ...
    for index,item in enumerate(protocol['schedule'],1):
        ...
        print(f'{index}/24 {case} {arm} exit={meta["exit_code"]} {meta["seconds"]}s',flush=True)
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('action',choices=['prepare','execute','analyze']);p.add_argument('--root',type=Path,required=True);p.add_argument('--baseline',type=Path)
